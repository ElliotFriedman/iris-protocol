...
contract CooldownEnforcerTest is Test {
    ...
    function test_firstExecutionPasses() public view {
        ...
        enforcer.beforeHook(terms, "", DM, DELEGATION_HASH, DELEGATOR, REDEEMER, TARGET, 1 ether, "");
    }

    function test_revertsWithinCooldown() public {
        bytes memory terms = abi.encode(COOLDOWN, THRESHOLD);
        enforcer.afterHook(terms, "", DM, DELEGATION_HASH, DELEGATOR, REDEEMER, TARGET, 1 ether, "");
        vm.warp(1100);
        vm.expectRevert(
    ...
    function test_belowThresholdBypassesCooldown() public {
        ...
        enforcer.afterHook(terms, "", DM, DELEGATION_HASH, DELEGATOR, REDEEMER, TARGET, 1 ether, "");
        vm.warp(1001);
        enforcer.beforeHook(terms, "", DM, DELEGATION_HASH, DELEGATOR, REDEEMER, TARGET, 0.1 ether, "");
    }

    function test_afterHookDoesNotRecordBelowThreshold() public {
        bytes memory terms = abi.encode(COOLDOWN, THRESHOLD);
        enforcer.afterHook(terms, "", DM, DELEGATION_HASH, DELEGATOR, REDEEMER, TARGET, 0.1 ether, "");
    ...
    function test_exactThresholdTriggersCooldown() public {
        bytes memory terms = abi.encode(COOLDOWN, THRESHOLD);
        enforcer.afterHook(terms, "", DM, DELEGATION_HASH, DELEGATOR, REDEEMER, TARGET, THRESHOLD, "");
        vm.warp(1100);
        vm.expectRevert();
        enforcer.beforeHook(terms, "", DM, DELEGATION_HASH, DELEGATOR, REDEEMER, TARGET, THRESHOLD, "");
    }
}
